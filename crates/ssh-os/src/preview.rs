//! Half-block image preview for the Viewer pane (unicode fallback).
//!
//! Prefer terminal graphics protocols (Sixel/Kitty/iTerm2) via `ratatui-image`
//! when available; this module is the portable color fallback (▀ cells).
...
impl HalfblockPreview {
    ...
    pub fn from_bytes_on(
        ...
        let rgba: RgbaImage = img
            .resize_exact(nw, nh, FilterType::CatmullRom)
            .to_rgba8();
        ...
        Ok(Self {
            ...
            height: cell_rows,
            rows,
            meta: format!("{ow}×{oh} → {cell_cols}×{cell_rows} halfblocks"),
        })
    }
}
...
mod tests {
    ...
    fn decodes_tiny_png() {
        ...
        assert_eq!(preview.rows[0][0].glyph, '▀');
    }
}
